test('WHATWG URL standard - auth url', () => {
    ...
    expect(testUrl.href).toBe('https://user:pass@sub.example.com:8080/p/a/t/h?query=string#hash');
});
...
test('WHATWG URL standard - auth no password', () => {
    ...
    expect(testUrl.hash).toBe('#hash');
    expect(testUrl.origin).toBe('https://sub.example.com:8080');
    expect(testUrl.href).toBe('https://user@sub.example.com:8080/p/a/t/h?query=string#hash');
});
